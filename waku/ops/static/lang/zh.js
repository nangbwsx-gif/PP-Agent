...
const ZH = {
  ...
  "conn.group.tools": "工具",
  // 微信连接卡：扫码、退出，以及它自己在做什么。
  "conn.wechat.scan": "扫码登录",
  "conn.wechat.logout": "退出登录",
  "conn.wechat.title": "登录微信",
  "conn.wechat.howto": "手机上：微信 → 右上角 + → 扫一扫 → 确认。这个页面会一直等，不用去终端敲命令。",
  "conn.wechat.getQr": "获取二维码",
  "conn.wechat.fetching": "正在向微信索取二维码…",
  "conn.wechat.scanNow": "用微信扫这个码，然后在手机上确认。",
  "conn.wechat.scaned": "已扫码 —— 请在手机上确认。",
  "conn.wechat.waiting": "等待扫码…",
  "conn.wechat.confirmed": "已登录，开始接收消息。",
  "conn.wechat.expired": "二维码过期了，再点一次「获取二维码」。",
  "conn.wechat.replaces": "这会替换当前绑定的账号（{id}）。",
  "conn.wechat.pending": "{n} 条回复尚未送达",
  "conn.wechat.unfinished": "{n} 条消息被中断",
  "conn.wechat.refused": "{n} 个发送者被拒绝",
  "conn.wechat.noId": "{n} 条消息缺少 ID",
  "conn.wechat.confirmLogout": "退出微信登录？在你重新扫码之前，bot 不会再回话。",
  ...
  "graph.intro": "循环就是一轮 Agent 行为：模型不停地选工具，直到它停下来。但有些活的<b>形状</b>是固定的 —— 有些步骤可以同时跑，还有些「如果这样，就去那里」的显式分叉。图谱工作流把这种形状变成一等公民：节点（各干一件事）用边（下一步去哪）连起来。循环一行都没改 —— 下面那个 <code>full_agent</code> 节点就是同一个循环，只是作为一个步骤在跑。骨架会自己给每条消息选路 —— 你也可以在对话框里按名字调用工作流：输入 <code>/graphs</code> 就能看到它们。",
  "graph.offTitle": "已关闭",
  ...
  "set.devTitle": "开发者模式",
  "set.devIntro": "显示那些只有自己动手折腾才用得上的页面：图谱、循环、运维与追踪、数据库和 SQL 控制台、两个竞速擂台。关闭时侧边栏留总览、记忆、工具、模型、连接、行为。切换后页面会重新加载 —— 侧边栏和当前页面都会变。这里的选择优先于启动参数 --dev，所以关掉就会一直关着。",
  "set.devLabel": "开发者模式",
  "set.devReload": "保存并重新加载页面",
